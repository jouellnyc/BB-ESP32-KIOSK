...
from hardware.esp32_oled_2_8_inch import red, black, white, drk_grn,
gc.collect()
print(gc.mem_free())

from hardware.esp32_oled_2_8_inch import score_font
gc.collect()
print(gc.mem_free())

"""
from hardware.esp32_oled_2_8_inch import date_font
gc.collect()
print(gc.mem_free())
"""

from hardware.esp32_oled_2_8_inch import sm_font
gc.collect()
print(gc.mem_free())
...
""" MLB SETUP """
from .my_mlb_api  import schedule
...
def get_score():
        ...
        if game_status == "In Progress" or "eview" in game_status:
            ...
            clear_fill()
            display.draw_text(0,  start + (0 * delta), f"{mt}-{dy}-{short_yr} {in_sta} {inn_cur}", sm_font,  white , drk_grn)
...
def no_gm():
    ...
    clear_fill()
    draw_outline_box()
    display.draw_text(5, 5, gm_dt, sm_font, white, drk_grn)
    display.draw_text(5, 75, 'No Game Today!', sm_font, white, drk_grn)
    print(f"No Game today!")
    print(f"ng -", gc.mem_free())
...
while True:
    ...
    try:
        ...
        games = schedule(start_date=gm_dt, end_date=gm_dt, team=team_id, params=params)
    except OSError as e:
