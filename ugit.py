...
#ignore_files = ["/ugit.py", "/ugit_log.py", "/hardware/wifi_config.py"]
ignore_files = ["/ugit.py", "/ugit_log.py", "hardware/wifi_config.py"]
ignore = ignore_files
...
call_trees_url = (
    f"https://api.github.com/repos/{user}/{repository}/git/trees/main?recursive=1"
)
...
def pull_all(tree=call_trees_url, raw=raw, ignore=ignore, isconnected=False):
    ...
    for i in tree["tree"]:
        if i["type"] == "tree":
            try:
                ...
                print(f'failed to mdkir {i["path"]} dir may already exist')
    ...
    Leave this stuff - JJO
    
        - ugit seems to rm '/hardware/wifi_config.py' here  if 'hardware/wifi_config.py' is in ignore above.
        - ugit seems to rm '/hardware/wifi_config.py'  above if '/hardware/wifi_config.py' is in ignore.
    ...
    if len(internal_tree) > 0:
        ...
        for i in internal_tree:
            os.remove(i)
            log.append(i + " removed at the end!")
    """
...
def add_to_tree(dir_item):
    ...
    if is_directory(dir_item) and len(os.listdir(dir_item)) >= 1:
        ...
        try:
            ...
            print(f"{dir_item} could not be added to tree {e}")
